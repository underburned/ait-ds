...
7. Если в пункте задания явно указано:
   - сравнить метрики &ndash; недостаточно вывести полученные цифры через принты или таблицей, необходимо кратко описать текстом, какая модель (метод) лучше и почему (метрики/критерии/хрустальный шар)
...
С визуализацией некоторых JS графиков может помочь [nbviewer](https://nbviewer.org/). Например, графики с использованием `Plotly` в 1 лекции и некоторые JS графики из 5 лекции отображаются в `nbviewer`, но графики, сгенерированные `CatBoost` &ndash; нет (хотя под капотом там тоже `Plotly`).  
